...
export default function FarmerMessages() {
  ...
  useEffect(() => {
    ...
    // WhatsApp-style live badge — bumps instantly, no refetch needed.
    socket.on("unread:update", ({ senderId }: { senderId: string; senderType: string }) => {
      setSelectedCompany((current) => {
        // If this thread is already open, the badge shouldn't bump —
        // getConversationByIds already marks it read server-side.
        if (current?.companyId === senderId) return current;
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          // Server already marked these read — clear the local badge too.
          setUnreadCounts((prev) => ({ ...prev, [selectedCompany.companyId]: 0 }));
        } else {
          setMessages([]);
        }
      } catch (err) {
      ...
      <div className="w-1/4 backdrop-blur-lg bg-white/60 border-r flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredCompanies.length === 0 && (
            <p className="text-center text-gray-400 text-sm mt-6">No companies found 🏢</p>
          )}

          {filteredCompanies.map((c) => {
            const unread = unreadCounts[c.companyId] || 0;
            ...
            return (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">{c.address || "No address info"}</p>
                </div>
