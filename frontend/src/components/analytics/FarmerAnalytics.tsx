...
const STATUS_COLORS: Record<string, string> = {
  ...
  escrow_funded: "bg-emerald-400",
  pending_admin_review: "bg-orange-400",
  admin_held: "bg-orange-500",
  shipment_released: "bg-emerald-500",
  delivery_released: "bg-emerald-600",
  disputed: "bg-red-500",
  rejected: "bg-red-400",
};
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        console.error("Fetch farmer analytics error:", err);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-gray-400">
                Revenue, buyers and on-chain audit trail
              </p>
            </div>
        ...
        <div className="space-y-6 px-5 py-5">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <div className="rounded-xl border border-gray-100 bg-gray-50/60 p-3">
              ...
              <p className="mt-1.5 text-lg font-bold text-gray-900">
                {formatCompactCurrency(data.summary.totalRevenue)}
              </p>
            </div>
          ...
          <div className="rounded-xl border border-gray-100 p-4">
            ...
            <p className="text-xs font-semibold text-gray-700">
              Revenue released over time
            </p>

            <div className="mt-3 h-48">
              {data.revenueOverTime.length === 0 ? (
                <div className="flex h-full items-center justify-center text-xs text-gray-400">
                  No released payments yet in this range.
                </div>
              ) : (
          ...
          {/* TOP CROPS + TOP BUYERS */}
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">

            <div className="rounded-xl border border-gray-100 p-4">
              <div className="flex items-center gap-1.5 text-gray-700">
                <Leaf size={14} />
                <p className="text-xs font-semibold">Top crops</p>
              </div>
                  ...
                  {data.topCrops.map((crop) => (
                    <div key={crop.cropName} className="flex items-center justify-between">
                      ...
                      <span className="shrink-0 text-xs font-semibold text-emerald-600">
                        {formatCompactCurrency(crop.revenue)}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-xl border border-gray-100 p-4">
              <div className="flex items-center gap-1.5 text-gray-700">
                <Building2 size={14} />
                <p className="text-xs font-semibold">Top buyers</p>
              </div>
                  ...
                  {data.topBuyers.map((buyer) => (
                    <div key={buyer.id} className="flex items-center justify-between">
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-gray-400">
                          {buyer.orders} order{buyer.orders === 1 ? "" : "s"}
                        </p>
                      </div>
                      <span className="shrink-0 text-xs font-semibold text-emerald-600">
                        {formatCompactCurrency(buyer.revenue)}
                      </span>
                    </div>
                  ))}
                </div>
